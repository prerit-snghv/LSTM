...
namespace {
    ...
    RtlOutputs run_rtl_model(const Inputs& in) {
        RtlOutputs out{};
        out.pre_i = mac_preact(in.w_i, in.u_i, in.b_i, in.x_t, in.h_prev);
        out.pre_f = mac_preact(in.w_f, in.u_f, in.b_f, in.x_t, in.h_prev);
        out.pre_g = mac_preact(in.w_g, in.u_g, in.b_g, in.x_t, in.h_prev);
        out.pre_o = mac_preact(in.w_o, in.u_o, in.b_o, in.x_t, in.h_prev);
        ...
        out.c_t = q12_add_sat(q12_mul_sat(out.f, in.c_prev), q12_mul_sat(out.i, out.g));
    ...
    IdealOutputs run_ideal_model(const Inputs& in) {
        const double x_t = to_real(in.x_t);
        const double h_prev = to_real(in.h_prev);
        const double c_prev = to_real(in.c_prev);

        IdealOutputs out{};
        out.pre_i = to_real(in.w_i) * x_t + to_real(in.u_i) * h_prev + to_real(in.b_i);
        out.pre_f = to_real(in.w_f) * x_t + to_real(in.u_f) * h_prev + to_real(in.b_f);
        out.pre_g = to_real(in.w_g) * x_t + to_real(in.u_g) * h_prev + to_real(in.b_g);
        out.pre_o = to_real(in.w_o) * x_t + to_real(in.u_o) * h_prev + to_real(in.b_o);
    ...
    void print_signal_compare(const std::string& name, int16_t rtl, double ideal) {
        ...
        std::cout << "  " << std::left << std::setw(8) << name
                ...
                << " rtl_real=" << std::setw(11) << std::fixed << std::setprecision(6) << to_real(rtl)
    ...
    void print_case(const Inputs& in) {
        const RtlOutputs rtl = run_rtl_model(in);
        const IdealOutputs ideal = run_ideal_model(in);

        std::cout << "case " << in.name << "\n";
        std::cout << "  raw inputs: x_t=" << in.x_t
                << " h_prev=" << in.h_prev
                << " c_prev=" << in.c_prev << "\n";
        std::cout << "  real inputs: x_t=" << std::fixed << std::setprecision(6) << to_real(in.x_t)
                << " h_prev=" << to_real(in.h_prev)
                << " c_prev=" << to_real(in.c_prev) << "\n";
        std::cout << "  compare RTL fixed-point vs ideal real math\n";

        print_signal_compare("pre_i", rtl.pre_i, ideal.pre_i);
        print_signal_compare("pre_f", rtl.pre_f, ideal.pre_f);
        print_signal_compare("pre_g", rtl.pre_g, ideal.pre_g);
        print_signal_compare("pre_o", rtl.pre_o, ideal.pre_o);
        print_signal_compare("i", rtl.i, ideal.i);
        print_signal_compare("f", rtl.f, ideal.f);
        print_signal_compare("g", rtl.g, ideal.g);
        print_signal_compare("o", rtl.o, ideal.o);
        print_signal_compare("c_t", rtl.c_t, ideal.c_t);
        print_signal_compare("tanh_c", rtl.tanh_c_t, ideal.tanh_c_t);
        print_signal_compare("h_t", rtl.h_t, ideal.h_t);
        std::cout << "\n";
    }

    std::vector<Inputs> default_cases() {
        return {
            {"nominal_positive", 4096, 2048, 1024,
            4096, 0, 0, 4096, 0, 0, 2048, 0, 0, 4096, 0, 0},
            {"negative_g", 4096, 2048, 1024,
            4096, 0, 0, 4096, 0, 0, -2048, 0, 0, 4096, 0, 0},
            {"nonzero_u", 4096, 2048, 1024,
            2048, 2048, 0, 4096, -2048, 0, 2048, 2048, 0, 2048, 2048, 0},
            {"nonzero_bias", 2048, 1024, 512,
            2048, 0, 1024, 2048, 0, -1024, 2048, 0, 512, 2048, 0, -512},
            {"near_saturation", 8192, 8192, 4096,
            8192, 8192, 4096, 8192, 8192, 4096, 8192, 8192, 0, 8192, 8192, 4096},
        };
    }
    ...
    void print_help(const char* argv0) {
        std::cout
            << "Usage:\n"
            << "  " << argv0 << "\n"
            << "  " << argv0 << " [--real-inputs] --case NAME --x X --h H --c C \\\n"
            ...
            << "Examples:\n"
            << "  " << argv0 << "\n"
            << "  " << argv0 << " --case tb_nominal --x 4096 --h 2048 --c 1024 \\\n"
            << "      --wi 4096 --ui 0 --bi 0 --wf 4096 --uf 0 --bf 0 \\\n"
            << "      --wg 2048 --ug 0 --bg 0 --wo 4096 --uo 0 --bo 0\n"
            << "  " << argv0 << " --real-inputs --case tb_nominal_real --x 1.0 --h 0.5 --c 0.25 \\\n"
    ...
    Inputs parse_custom_case(int argc, char** argv) {
        ...
        for (int index = 1; index < argc; ++index) {
            ...
            if (is_option(arg, "--case")) {
                ...
                in.x_t = parse_value(value);
                seen_x = true;
            } else if (is_option(arg, "--h")) {
                in.h_prev = parse_value(value);
                seen_h = true;
            } else if (is_option(arg, "--c")) {
                in.c_prev = parse_value(value);
                seen_c = true;
            } else if (is_option(arg, "--wi")) {
                in.w_i = parse_value(value);
                seen_wi = true;
            } else if (is_option(arg, "--ui")) {
                in.u_i = parse_value(value);
                seen_ui = true;
            } else if (is_option(arg, "--bi")) {
                in.b_i = parse_value(value);
                seen_bi = true;
            } else if (is_option(arg, "--wf")) {
                in.w_f = parse_value(value);
                seen_wf = true;
            } else if (is_option(arg, "--uf")) {
                in.u_f = parse_value(value);
                seen_uf = true;
            } else if (is_option(arg, "--bf")) {
                in.b_f = parse_value(value);
                seen_bf = true;
            } else if (is_option(arg, "--wg")) {
                in.w_g = parse_value(value);
                seen_wg = true;
            } else if (is_option(arg, "--ug")) {
                in.u_g = parse_value(value);
                seen_ug = true;
            } else if (is_option(arg, "--bg")) {
                in.b_g = parse_value(value);
                seen_bg = true;
            } else if (is_option(arg, "--wo")) {
                in.w_o = parse_value(value);
                seen_wo = true;
            } else if (is_option(arg, "--uo")) {
                in.u_o = parse_value(value);
                seen_uo = true;
            } else if (is_option(arg, "--bo")) {
                in.b_o = parse_value(value);
                seen_bo = true;
            } else {
        ...
        if (!(seen_x && seen_h && seen_c &&
            seen_wi && seen_ui && seen_bi &&
            seen_wf && seen_uf && seen_bf &&
            seen_wg && seen_ug && seen_bg &&
            seen_wo && seen_uo && seen_bo)) {
            throw std::runtime_error("custom case is missing one or more required inputs; run with --help");
        }
